...
export default function GameDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      const matches = matchesResponse.data.filter(
        (match: any) => match.jogo === id
      );
      ...
      const averages = categories.map((category) => {
        ...
        const sum = evaluations.reduce(
          (acc: number, curr: any) => acc + (curr[apiField] || 0),
          0
        );
      ...
      const totalAverage =
        evaluations.reduce((acc: number, curr: any) => acc + curr.nota, 0) /
        evaluations.length;
